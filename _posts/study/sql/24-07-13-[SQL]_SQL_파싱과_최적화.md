...
SQL은 'Structured Query Language'의 줄임말이다. 한글로는 구조적 질의 언어다.<br>
SQL은 기본적으로 구조적(structured)이고 집합적(set-based)이고 선언적(declarative)인 질의 언어다.<br>
원하는 결과 집합을 구조적, 집합적으로 선언하지만 그 결과집합을 만드는 과정은 절차적일 수밖에 없다. 이러한 과정을 수행하는 것이 DBMS 내부 엔진인 SQL 옵티마이저다.<br>
...
SQL 최적화는 다음과 같은 과정을 거친다.<br>
1. SQL 파싱: 사용자로부터 SQL을 전달받으면 가장 먼저 SQL 파서(Parser)가 파싱을 진행한다. 이 때 문법적 오류나 의미적 오류가 없는지 확인한다.<br>
2. SQL 최적화: 옵티마이저가 다양한 실행 경로를 생성해 비교한 후 가장 효율적인 하나를 선택한다.<br>
3. 로우 소스 생성: 옵티마이저가 선택한 실행경로를 실제 실행 가능한 코드 또는 프로시저 형태로 포매팅한다.<br>
